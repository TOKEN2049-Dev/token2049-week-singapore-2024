...
export default async function getEvents(getAllEvents = false, eventId = null) {
	try {
		...
		if (getAllEvents) query = `SELECT * FROM Events`;
		else if (eventId) query = `SELECT * FROM Events WHERE event_id=${eventId}`;
...
export async function checkIfClashingFeature(eventDate, eventId) {
	try {
		const query =
			"SELECT COUNT(*) AS eventCount FROM Events WHERE featured_event=1 AND event_date='" +
			eventDate +
			"' AND event_id!=" +
			eventId +
			" GROUP BY event_date;";
